...
asignacionT(jOrd,jobs)

"""###Algoritmo profesor"""

def split(secu,numM):
  comb=[line[0] for line in secu]
  tamaños=[line[2] for line in secu]
  if numM==1:
    ss=comb
  else:
    k=0
    ...
    for i in range(len(comb)-numMaq):
      if sP[i] >= int(sP[len(comb)-1]/numMaq):
        ss[0]= comb[0:i]
        k=i
        break     
    h=1
    for i in range(k+1,len(comb)):
      if h== numMaq-1:
        break
      if sP[i]-sP[k]>= int((sP[len(comb)-1]-sP[k])/(numMaq-h)):
        ss[h] = comb[k:i+1]
        h+=1
        k=i  
    ss[h] = comb[k:len(comb)]
...
def asignacionBatches(jobs,jOrd,numMaq):
  sec= [i[0] for i in jOrd]
  batchesMaq= batches(sec,jobs,maq) 
...
asignacionBatches(jobs,jOrd,numMaq)
...
sec1=split(randomOrd(jobs),numMaq)
...
sec2=split(ruidoOrd(jobs,50),numMaq)
